...
function BaseLayout(req, meta, bodyContent, scriptContent = '', currentExp = 'architect') {
    ...
    return `<!DOCTYPE html>
<html lang="en" class="scroll-smooth"${initialTheme ? ` data-theme="${initialTheme}"` : ''} data-experience="${currentExp}">
<head>
    <meta charset="UTF-8">
...
<body class="min-h-screen flex flex-col bg-velora-bg text-velora-text relative">
    ...
    <script>
        ...
        // Mobile Experience Selector Drawer Toggle
        const expToggle = document.getElementById('mobile-experience-toggle');
        const expMenu = document.getElementById('mobile-experience-menu');
        if (expToggle && expMenu) {
            expToggle.addEventListener('click', function(e) {
                e.stopPropagation();
                const isHidden = expMenu.classList.toggle('hidden');
                expToggle.setAttribute('aria-expanded', !isHidden);
            });
            document.addEventListener('click', function(e) {
                if (!expMenu.classList.contains('hidden') && !expMenu.contains(e.target) && !expToggle.contains(e.target)) {
